...
<body>
    <form class="form-horizontal" action="{{ route('manager-sports-update', $sport->id) }}" method="POST"
        ...
        <fieldset>
            ...
            <div class="form-group">
                ...
                <div class="col-md-4">
                    <input id="product_name" name="name" value="{{ old('name') ? old('name') : $sport->name }}"
                        placeholder="SPORT NAME" class="form-control input-md" type="text">
                    ...
                    <select id="product_categorie" name="category_id" class="form-control">
                        @foreach ($categories as $category)
                            <option value="{{ $category->id }}"
                                {{ old('category_id', $sport->category_id) == $category->id ? 'selected' : '' }}>
                                {{ $category->name }}
                            </option>
                        @endforeach
                    </select>
            ...
            <div class="form-group">
                ...
                <div class="col-md-4">
                    <input name="image_path" value="{{ $sport->image_path }}" id="filebutton" placeholder="image"
                    ...
                    <select id="product_categorie" name="price_id" class="form-control">
                        @foreach ($prices as $price)
                            <option value="{{ $price->id }}"
                                {{ old('price_id', $sport->price_id) == $price->id ? 'selected' : '' }}>
                                {{ $price->price }}
                            </option>
                        @endforeach
                    </select>
            ...
            <div class="form-group">
                ...
                <div class="col-md-4">
                    ...
                    @foreach ($images as $image)
                        <form action="/manager-deleteimage/{{ $image->id }}" method="post"
                            enctype="multipart/form-data">
